...
AForm::AForm(std::string name, int signGrade, int executeGrade)
    ...
    if (signGrade < Bureaucrat::highestGrade ||
        executeGrade < Bureaucrat::highestGrade)
        throw AForm::GradeTooHighException();
    if (signGrade > Bureaucrat::lowestGrade ||
        executeGrade > Bureaucrat::lowestGrade)
        throw AForm::GradeTooLowException();
}

AForm::AForm(const AForm &form)
    : name(form.name), signGrade(form.signGrade),
      executeGrade(form.executeGrade), isSigned(form.isSigned) {}
...
void AForm::beSigned(const Bureaucrat &bureaucrat) {
    if (bureaucrat.getGrade() > this->signGrade)
        ...
        std::cout << bureaucrat.getName() << " could not sign "
                  << this->getName() << " because it is already signed.\n";
    else {
...
std::ostream &operator<<(std::ostream &os, const AForm &form) {
    os << "Form " << form.getName() << " is signable by grade "
       << form.getSignGrade() << " and executable by grade "
       << form.getExecuteGrade() << ". It is"
